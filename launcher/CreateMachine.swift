...
internal class CreateMachineCommand: Command {
	...
	internal static func runDockerMachineCreate(logHandle: FileHandle) throws {
		...
		if !fm.directoryExists(atPath: "/Library/ServiceData/Docker/docker-machine/machines/default") {
			...
			os_log(.default, log: log, "Docker machine successfully created")

			// Now we have to reset the permissions on the directories
			// generated by docker-machine, or the user won't be able
			// to connect to the VM due to permission-denied errors.
			try Task.run(bash: "find /Library/ServiceData/Docker/docker-machine -type d | xargs chmod 755")
			try Task.run(bash: "find /Library/ServiceData/Docker/docker-machine -type f | xargs chmod 644")
		}
	}
